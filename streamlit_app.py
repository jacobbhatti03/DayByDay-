# app.py
"""
DayByDay — Streamlit single-file app (~600 lines)
- Local username/password auth
- Persistent session across reloads (session.json)
- Gemini AI integration (tries multiple SDK shapes) with fallback
- 8-day AI-generated plan parsed into editable day cards
- Separate Chat tab for DayBot
- Full task dicts with stable IDs
- No experimental rerun usage
"""
...
_session_data = read_json(SESSION_FILE, {})
if _session_data.get("user") and not st.session_state.user:
    st.session_state.user = _session_data.get("user")
    st.session_state.project = _session_data.get("project", st.session_state.project)
    st.session_state.chat_history = _session_data.get("chat_history", [])
    st.session_state.page = _session_data.get("page", "home")
    ...
    for line in lines:
        ...
        if line.lower().startswith("day "):
            for token in line.split():
                if token.isdigit():
                    current_day = int(token) - 1
                    break
            continue
        if current_day is not None and 0 <= current_day < 8:
            cleaned = line.lstrip("-•0123456789.). \t")
            if cleaned:
                days[current_day].append({"id": None, "text": cleaned, "done": False})
    # assign IDs
    nid = 0
    for d in range(8):
        for t in days[d]:
...
def generate_8day_plan(title: str, desc: str):
    ...
    if not ok:
        return False, f"DayBot error: {ai_text}", None

    # 2️⃣ Split into Day 1..Day 8 blocks
    days_raw = re.split(r"(?=Day\s*\d+[:\-])", ai_text, flags=re.IGNORECASE)
    day_tasks = [[] for _ in range(8)]

    for i in range(1, 9):
        # find corresponding day block
        block = next((d for d in days_raw if re.match(fr"Day\s*{i}\b", d, flags=re.IGNORECASE)), "")
        if block:
            content = re.sub(fr"Day\s*{i}[:\-]?", "", block, flags=re.IGNORECASE).strip()
            # split by lines and clean bullets
            lines = [line.lstrip("-•0123456789.). \t").strip() for line in content.splitlines() if line.strip()]
            day_tasks[i-1] = lines or ["(No tasks generated)"]
        else:
            day_tasks[i-1] = ["(No tasks generated)"]

    # 3️⃣ Normalize tasks into dicts with unique ids
    normalized_days = []
    nid = 0
    for day in day_tasks:
        norm_tasks = []
        for t in day:
            norm_tasks.append({"id": nid, "text": t, "done": False})
            nid += 1
        normalized_days.append(norm_tasks)

    # 4️⃣ Create project dict and save
    project = {
        "title": title.strip(),
        "description": desc.strip(),
        "tasks": normalized_days,
        "generated_at": datetime.utcnow().isoformat(),
    }

    # Save project in session + file
    st.session_state.project = project
    save_user_project(st.session_state.user, project)

    # 5️⃣ Switch to planner page
...
st.markdown('<div class="header"><h2 style="margin:0">📅 DayByDay</h2><div class="small">Your AI project planner — DayBot</div></div>', unsafe_allow_html=True)
...
def page_home():
    ...
    with left:
        ...
        if not projects:
            ...
            selected = st.selectbox("Open a project", options=["-- select --"] + list(projects.keys()), key="home_proj_sel")
            if selected != "-- select --":
...
def normalize_tasks(raw_tasks):
    ...
    for t in raw_tasks:
        if isinstance(t, str):
            normalized.append({"id": nid, "text": t.strip(), "done": False})
        ...
        with st.expander(f"**Day {i+1}**", expanded=(i==0)):
            if tasks[i]:
                for j, t in enumerate(tasks[i]):
                    ...
                    c1.markdown(f"- {t.get('text') if isinstance(t, dict) else t}")
            ...
            if st.button(f"Add to Day {i+1}", key=f"add_btn_{i}"):
                if new_task.strip():
                    nid = max([tt.get("id", -1) for dd in tasks for tt in dd]+[-1])+1
...
def page_chat():
    ...
    st.markdown("<div class='small'>Ask DayBot project-related questions or improve tasks.</div>", unsafe_allow_html=True)
    if st.session_state.ask_context:
        ctx = st.session_state.ask_context
        now = datetime.utcnow().isoformat()
        prompt = f"Project: {ctx.get('project_title','')}\nContext: Day {ctx.get('day',0)+1}\nTask: {ctx.get('task_text','')}\nProvide 2 improved alternatives with reason."
        ok, reply = ask_daybot_contextual(st.session_state.project.get("raw_plan","") or st.session_state.project.get("description",""), prompt)
        st.session_state.chat_history.append({"role":"user","text":f"Improve task: {ctx.get('task_text','')}", "time": now})
        if ok:
            st.session_state.chat_history.append({"role":"daybot","text":reply,"time":now})
        else:
            st.session_state.chat_history.append({"role":"daybot","text":"DayBot unavailable.","time":now})
        st.session_state.ask_context = None
        persist_session()
    for msg in st.session_state.chat_history[-100:]:
        if msg.get("role")=="user":
            st.markdown(f"**You ({msg.get('time')[:19]}):** {msg.get('text')}")
        else:
            st.markdown(f"**DayBot ({msg.get('time')[:19]}):** {msg.get('text')}")
    st.markdown("---")
    ...
    if st.button("Send", key="chat_send"):
        if not user_msg.strip():
            ...
            now = datetime.utcnow().isoformat()
            st.session_state.chat_history.append({"role":"user","text":user_msg.strip(),"time":now})
    ...
    if st.session_state.chat_history:
        ...
        if last.get("role")=="daybot" and "Day" in last.get("text","") and st.button("Import last DayBot reply into tasks"):
            parsed = parse_plan_to_tasks(last.get("text",""))
            proj = st.session_state.project
            all_ids = [tt.get("id") for dd in proj.get("tasks",[]) for tt in dd if tt.get("id") is not None]
            nid = max(all_ids)+1 if all_ids else 0
            for i in range(8):
                for t in parsed[i]:
                    text = t.get("text") if isinstance(t, dict) else str(t)
                    proj["tasks"][i].append({"id":nid,"text":text,"done":False})
                    nid += 1
            save_user_project(st.session_state.user, proj)
            persist_session()
...
def render_sidebar():
    with st.sidebar:
        ...
        try:
            ...
            idx = 0
        sel = st.radio("Navigate", labels, index=idx, key="sidebar_nav", label_visibility="collapsed")
